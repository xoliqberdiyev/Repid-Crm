...
@common_router.get('/income-expected-value', response_model=Page[schemas.ShowExpectedValue])
...
@common_router.get('/expense-expected-val', response_model=Page[schemas.ShowExpectedValue])
async def get_all_income_val(db:AsyncSession = Depends(session.get_db),current_user:models.Employees=Depends(get_current_user_from_token)):
    expense_list =  await common_action._get_all_expense_expected_value(session=db)
    return paginate(expense_list)

@common_router.post('/expected-value', response_model=schemas.ShowExpectedValue)
...
@common_router.delete('/expected-value')
...
@common_router.patch('/expected-value',response_model=schemas.ShowExpectedValue)
...
@common_router.get('/task', response_model=List[schemas.ShowNewTask])
...
@common_router.post('/task', response_model=schemas.ShowNewTask)
...
@common_router.patch('/task-status')
...
@common_router.patch('/task',response_model=schemas.ShowNewTask)
...
@common_router.get('/operator-type', response_model=List[schemas.ShowPosition])
...
@common_router.post('/operator-type',response_model=schemas.ShowPosition)
...
@common_router.patch('login-password-note', response_model=schemas.ShowLoginPassword)
...
@common_router.get('/pocket-value')
