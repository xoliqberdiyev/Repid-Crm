...
async def _update_expense_employee(pay_price:str, expense_id:int, session:AsyncSession):
    try:
        async with session.begin():
            in_ex_dal = income_expense_dal.IncomeExepnseDal(session)

            update_expense_employee = await in_ex_dal.update_expense_employee(pay_price=pay_price,
